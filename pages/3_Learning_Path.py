...
# -----------------------------------------------------
    ...
    if is_topic_mastered:
        ...
        if next_topic_index < len(topic_ids):
            ...
            st.success("You have reached the end of the modules.")
...
# -----------------------------------------------------
    ...
    if st.button("Submit Answer"):
        ...
        if is_correct:
            st.balloons()
            st.success("✅ Theory Verified! Proceeding to Practical Mastery...")
...
# ==========================================
    ...
    if CODING_CHALLENGE_KEY not in st.session_state:
        with st.spinner("Crafting a challenge for you..."):
            current_theta = progress_data['irt_theta_initial']
            user_level = helpers.get_ability_level(current_theta)
    ...
    if CODING_FEEDBACK_KEY in st.session_state:
        ...
        if feedback['is_correct']:
            ...
            st.info("You have demonstrated mastery of this topic. The next lesson is now unlocked.")
            
            if st.button("Continue to Next Topic ➡️"):
                # --- MASTERY MOMENT ---
                if not is_topic_mastered:
                ...
                if next_topic:
                    ...
                    st.balloons()
                    st.success("You have completed the curriculum!")
                    db.update_progress(user_id, subject, status='assessing')
                    st.page_link("pages/4_Assignments.py", label="Final Assessment")
        else:
